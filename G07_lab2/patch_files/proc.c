...
// Uncomment DEFAULT to run original scheduler and HYBRID to run hybrid scheduler using sorting and queue
...
//    acquire(&pqueue.lock);
//	  insert_pqueue(*p);
//	  pqueue.pidmap[p->pid] = (p - &ptable.proc[0])/sizeof(ptable.proc[0]);
//	  release(&pqueue.lock);
...
//    acquire(&pqueue.lock);
//	  insert_pqueue(*np);
//	  pqueue.pidmap[np->pid] = (np - &ptable.proc[0])/sizeof(ptable.proc[0]);
//	  release(&pqueue.lock);
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      switchuvm(p);
      p->state = RUNNING;
      p->numcntxtswtchs=p->numcntxtswtchs+1;
...
void sort_queue(struct proc* arr[], int len){
  // simple bubble sort
  int n = len;

  for(int i=0;i<n;i++){
    for(int j=0;j+1<n-i;j++){
      if(arr[j]->bursts > arr[j+1]->bursts){
...
/*
	...
	while(pqueue.heapSize > 0) {
		struct proc lowBursts = pop_pqueue();   //Complexity of pop_pqueue() is O(logn)
...
int getMaxPid()
{
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    if(MaxPid < p->pid && p->state!=UNUSED)	
      MaxPid = p->pid;
  }
...
int getProcInfo(int Pid, struct processInfo* process_a){
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    if(p->pid==Pid) 
    {
      process_a->ppid=p->parent->pid;
      process_a->psize=(int)p->sz;
      process_a->numberContextSwitches=p->numcntxtswtchs;
      release(&ptable.lock);
      return 0;
    }
  }	
...
cps()
{
  ...
  cprintf("name \t pid \t state \t bursts\n");
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->state == UNUSED) continue;
    cprintf("%s\t %d \t %s \t %d\n", p->name, p->pid, states[p->state], p->bursts);
  }
